...
function editStatusNotes(orderId, status) {
    return dispatch => {
        ...
        notesService.editStatusNotes(orderId, status)
            .then(
                res => {
                    if (res === undefined) {
                        dispatch(alertActions.error('ارتباط با سرور برقرار نیست'));
                        dispatch(failure(notesConstants.EDIT_NOTES_STATUS_FAILURE, 'ارتباط با سرور برقرار نیست'));
                    } else if (res.success) {
                        console.log("user into notesActions");
                ...
                error => {
                    dispatch(failure(notesConstants.EDIT_NOTES_STATUS_FAILURE, error.toString()));
                    console.log("occure error");
                    console.log(error.toString());
                    dispatch(alertActions.error(error.toString()));
                }
