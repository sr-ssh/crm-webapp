    ...
    return (
        ...
        >
            <Modal.Body className="add-product px-4" style={{ maxHeight: documents?.length == 0 ? null : "80vh" }}>
                ...
                <Card className="m-auto mt-2 mb-2 bg-light productCard lh-lg " style={{ maxHeight: "70vh" }} >
                    <Card.Body className={`card--body--show--doc--mobile p-3 px-4 mx-2 rounded-3 fs-6-sm ${documentsLoading && 'd-flex justify-content-center'} `}>
                        {props.show && !documentsLoading && documents?.length == 0 && <><h6 className="mt-5 text-center">هنوز مدرکی برای این سفارش اضافه نشده است</h6><br /> <h6 className="mb-5 text-center text--dark--blue" style={{ cursor: "pointer" }} onClick={() => { props.onHide(false); setProductNameValidated(false); props.setActiveOrder(props.order); props.UploadModalShow() }}>بارگذاری مدرک</h6>  </>}
                        ...
                        <Row >
                            {
                                props.show ?
                                    documents?.length > 0 && documents?.map((doc, index) => {
                                        ...
                                        console.log(extname)
                                        return (<Col key={index} xs={4} className="pb-3 px-4">
                                            <Col>{doc.name}</Col>
                                            <Col>
