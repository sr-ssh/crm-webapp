                ...
                <Card.Body className="pb-0 pt-0 px-0 lh-lg fs-9 text-center fw-bold">
                    <Row className="m-0 p-0">
                        <Table bordered size="sm" className="table--blue mb-0">
                            <thead>
                                <tr>
                                    <th style={{ 'width': '6%' }} className="px-0 py-2">ردیف</th>
                                    <th style={{ 'width': '25.6%' }} className="px-0 py-2">شرح کالا یا خدمات</th>
                                    <th style={{ 'width': '6%' }} className="px-0 py-2">تعداد</th>
                                    <th style={{ 'width': '13.4%' }} className="px-0 py-2">مبلغ واحد (ریال)</th>
                                    <th style={{ 'width': '12.19%' }} className="px-0 py-2">تخفیف</th>
                                </tr>
                            </thead>
                            <tbody>
                                {console.log(order.products)}
                                {
                                    order.products?.map((product, index) =>
                                        <tr>
                                            <td className="py-2">{(index + 1) && persianJs(index + 1).englishNumber().toString()}</td>
                                            <td className="py-2">{product.name}</td>
                                            <td className="py-2">{product.quantity && persianJs(commaNumber(product.quantity)).englishNumber().toString()}</td>
                                            <td className="py-2">{product.sellingPrice && persianJs(commaNumber(product.sellingPrice)).englishNumber().toString()}</td>
                                            <td className="py-2">{persianJs(commaNumber(0)).englishNumber().toString()}</td>
                                        </tr>
                                    )
                                }
                            </tbody>
